...
/* =========================================================
   HabeshaGo Premium Frontend Safety + Visual Overrides
   These functions intentionally override earlier demo functions
   without changing backend endpoints.
   ========================================================= */

state.selectedMerchant = state.selectedMerchant || null;
...
function getLatLng(item) {
  ...
  const lng = safeNumber(item?.longitude ?? item?.lng);
  if (lat === null || lng === null) return null;
  return { lat, lng };
}

function merchantImage(merchant, index = 0) {
  const category = String(merchant?.category || "restaurant").toLowerCase();
  const images = {
    restaurant: "https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=900&q=80",
    ...
    supermarket: "https://images.unsplash.com/photo-1604719312566-8912e9227c6a?auto=format&fit=crop&w=900&q=80",
    marketplace: "https://images.unsplash.com/photo-1511690743698-d9d85f2fbf38?auto=format&fit=crop&w=900&q=80",
  ...
  return images[category] || fallback[index % fallback.length];
}

function productImage(product, index = 0) {
  const name = String(product?.name || "").toLowerCase();
  if (name.includes("coffee") || name.includes("buna")) return "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?auto=format&fit=crop&w=300&q=80";
  if (name.includes("pizza")) return "https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?auto=format&fit=crop&w=300&q=80";
  if (name.includes("burger")) return "https://images.unsplash.com/photo-1550547660-d9450f859349?auto=format&fit=crop&w=300&q=80";
  if (name.includes("injera") || name.includes("tibs") || name.includes("kitfo")) return "https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=300&q=80";
  const imgs = [
...
function selectMerchantById(merchantId) {
  ...
  document.querySelectorAll(".merchant-card-premium").forEach((card) => {
    card.classList.toggle("selected", card.dataset.merchantId === merchantId);
  });
  toast(`Selected ${merchant.name}`);
  renderMerchantDetails();
}
...
function renderMerchants() {
  ...
  if (!list) return;
  if (!Array.isArray(state.merchants) || !state.merchants.length) {
    list.innerHTML = "<div class='card'><strong>No restaurants found</strong><span>Try changing your search or increasing the delivery radius.</span></div>";
    return;
  }

  if (!state.selectedMerchant) {
    state.selectedMerchant = state.merchants.find((merchant) => merchant?.id) || null;
  }

  list.innerHTML = state.merchants.map((merchant, merchantIndex) => {
    const products = state.products.filter((product) => product.merchant_id === merchant.id);
    const isSelected = state.selectedMerchant?.id === merchant.id;
    const eta = merchant.distance_km != null ? `${Math.max(15, Math.round(Number(merchant.distance_km) * 12 + 18))}-${Math.max(25, Math.round(Number(merchant.distance_km) * 12 + 30))} min` : "25-40 min";
    return `
      <article class="card merchant-card-premium ${isSelected ? "selected" : ""}" data-merchant-id="${merchant.id}">
        <div class="merchant-image" style="background-image:url('${merchantImage(merchant, merchantIndex)}')">
          <span class="merchant-favorite">♡</span>
        </div>
        <div class="merchant-body">
          <h3>${merchant.name || "Unnamed merchant"}</h3>
          ...
          <div>
            <span class="badge">${merchant.category || "restaurant"}</span>
            ${merchant.women_owned ? '<span class="badge">Almaz</span>' : ""}
            ${merchant.verified ? '<span class="badge">Verified</span>' : ""}
            <span class="badge">${merchant.city_id || "Addis Ababa"}</span>
          </div>
          <button class="secondary" data-select-merchant="${merchant.id}" type="button">View menu</button>
          ${products.length ? products.map((product, productIndex) => `
            <div class="product">
              <div class="product-thumb" style="background-image:url('${productImage(product, productIndex)}')"></div>
              <span>
                <strong>${product.name}</strong><br>
                <small>${product.description || "Fresh local favorite prepared by the merchant."}</small><br>
                <small>${money(product.price, product.currency)} • ${product.prep_time_minutes || 15} min • stock ${product.stock_quantity ?? "n/a"}${product.popular ? " • popular" : ""}</small>
              </span>
              <button data-add="${product.id}" type="button">Add</button>
            </div>
          `).join("") : "<p>No menu items loaded yet.</p>"}
        </div>
      </article>
    `;
  }).join("");
  ...
  document.querySelectorAll("[data-select-merchant]").forEach((button) => {
    button.addEventListener("click", () => selectMerchantById(button.dataset.selectMerchant));
  });
  document.querySelectorAll("[data-add]").forEach((button) => {
    button.addEventListener("click", () => runButtonAction(button, "Add item to cart", () => addToCart(button.dataset.add)).catch(() => {}));
  });
...
function renderMerchantDetails() {
  const target = $("#merchantDetails");
  if (!target) return;
  const merchant = state.selectedMerchant || state.merchants.find((item) => item?.id);
  if (!merchant) {
    target.innerHTML = "<div class='detail'><span>Merchant</span><strong>Select a restaurant to view details</strong></div>";
    return;
  }
  target.innerHTML = [
    detail("Selected restaurant", merchant.name),
    detail("Category", merchant.category),
    detail("Rating", `${merchant.rating ?? 0} (${merchant.review_count || 0} reviews)`),
    detail("Distance", merchant.distance_km != null ? `${merchant.distance_km} km` : "Location pending"),
    detail("Hours", merchant.opening_hours || "Open today"),
    detail("Prep time", `${merchant.prep_time_minutes || 20} min`),
    detail("Delivery radius", `${merchant.delivery_radius_km || 3} km`),
    detail("Verification", merchant.verification_status || (merchant.verified ? "verified" : "pending"))
  ].join("");
}
...
async function sendMenuRequest() {
  ...
  if (!merchant?.id) {
    ...
    showActionStatus({ title: "Send menu request", message: "Please select a restaurant before sending a menu request.", type: "error" });
    return;
  }
  await api(`/api/${state.country}/v1/menu-requests`, {
    method: "POST",
  ...
  toast(`Menu request sent to ${merchant.name}`);
  await refreshAll();
}
...
function renderLeafletMap(data) {
  ...
  const origin = getLatLng(data?.origin) || selectedMapOrigin();
  if (!origin || !Number.isFinite(origin.lat) || !Number.isFinite(origin.lng)) {
    setMapNotice("Map origin is invalid. Please choose a delivery location.", "error");
    return;
  }
  ...
  addLayer(L.marker([origin.lat, origin.lng], { icon: mapIcon("You", "#0b7a3b") }).bindPopup("Customer location"));
  addLayer(L.circle([origin.lat, origin.lng], {
    radius: Number($("#nearbyRadius")?.value || 5) * 1000,
    color: "#0b7a3b",
    fillColor: "#0b7a3b",
  ...
  (data?.merchants || []).slice(0, 8).forEach((merchant) => {
    const point = getLatLng(merchant);
    if (!point) {
      console.warn("Skipping merchant with invalid coordinates", merchant);
      return;
    }
    addLayer(L.marker([point.lat, point.lng], { icon: mapIcon("Shop", "#f59e0b") }).bindPopup(`${merchant.name}<br>${merchant.category || ""} - ${merchant.distance_km ?? "?"} km<br>${merchant.address_note || ""}`));
  });

  (data?.drivers || []).slice(0, 8).forEach((driver) => {
    const point = getLatLng(driver);
    if (!point) {
      console.warn("Skipping driver with invalid coordinates", driver);
      return;
    }
    addLayer(L.circleMarker([point.lat, point.lng], {
      radius: 9,
      color: "#0f3d2a",
      fillColor: "#16a34a",
      fillOpacity: 0.88
  ...
  const route = (data?.demo_route || []).map(getLatLng).filter(Boolean);
  if (route.length) {
    addLayer(L.polyline(route.map((point) => [point.lat, point.lng]), { color: "#0f3d2a", weight: 5, opacity: 0.78 }).bindPopup("Simulated driver delivery route"));
  }

  (data?.recent_locations || []).slice(-10).forEach((point) => {
    const latLng = getLatLng(point);
    if (!latLng) {
      console.warn("Skipping movement ping with invalid coordinates", point);
      return;
    }
    addLayer(L.circleMarker([latLng.lat, latLng.lng], {
      radius: 4,
      color: "#061512",
      fillColor: "#061512",
      fillOpacity: 0.6
    }).bindPopup(`Movement ping${point.order_id ? `<br>Order ${point.order_id.slice(0, 8)}` : ""}<br>${point.created_at ? new Date(point.created_at).toLocaleTimeString() : ""}`));
  });
}
